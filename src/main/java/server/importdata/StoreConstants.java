package server.importdata;

public class StoreConstants {
    public static final int REPLACE_JOURNAL_STRATEGY = 0;
    public static final int IGNORE_JOURNAL_STRATEGY = 1;
    public static final int EXCEPTION_THROW_JOURNAL_STRATEGY = 2;
    public static final int REPLACE_TASK_STRATEGY = 3;
    public static final int IGNORE_TASK_STRATEGY = 4;
    public static final int EXCEPTION_THROW_TASK_STRATEGY = 5;
}
