...
public class StoreStrategyHelper {
    ...
    public boolean resolveStrategy(int strategy, Object object) throws StoreException {
        switch (strategy) {
            case StoreConstants.REPLACE_JOURNAL_STRATEGY:
                return strategies.get(StoreConstants.REPLACE_JOURNAL_STRATEGY).store(object);
            case StoreConstants.IGNORE_JOURNAL_STRATEGY:
                return strategies.get(StoreConstants.IGNORE_JOURNAL_STRATEGY).store(object);
            case StoreConstants.EXCEPTION_THROW_JOURNAL_STRATEGY:
                return strategies.get(StoreConstants.EXCEPTION_THROW_JOURNAL_STRATEGY).store(object);
            case StoreConstants.REPLACE_TASK_STRATEGY:
                return strategies.get(StoreConstants.REPLACE_TASK_STRATEGY).store(object);
            case StoreConstants.IGNORE_TASK_STRATEGY:
                return strategies.get(StoreConstants.IGNORE_TASK_STRATEGY).store(object);
            case StoreConstants.EXCEPTION_THROW_TASK_STRATEGY:
                return strategies.get(StoreConstants.EXCEPTION_THROW_TASK_STRATEGY).store(object);
            default:
                return false;
        }
    }
